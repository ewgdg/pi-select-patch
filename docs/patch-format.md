...
- Within one Update File section, every hunk resolves and validates against the same immutable pre-edit source before any output materializes. After anchor affinity and smart-selector dominance determine each hunk's strongest candidate set, consecutive tied hunks resolve as a local group. Complete assignments use full candidate sets and reject every overlap, including one with a uniquely resolved hunk or an earlier resolved group. One conflict-free assignment applies regardless of source order; no conflict-free assignment reports conflicting hunks. With multiple assignments, authored source order is a tie-breaker only when exactly one complete assignment follows the group's hunk sequence and its usable nearest uniquely resolved predecessor/successor boundaries; reverse-source or mutually incompatible boundaries cannot disable an otherwise locally resolvable group. Source-order resolutions appear only in `orderResolution` hunk audit metadata without a warning; each resolved hunk records its ambiguity-group bounds and selected pre-edit source span. Hunk candidate and ambiguity-search limits fail as `[E_HUNK_CANDIDATE_LIMIT]`; a limit never establishes uniqueness from truncation. Hunks then materialize in authored order, never matching inserted output; use a later Update File section when an edit must depend on earlier output.
